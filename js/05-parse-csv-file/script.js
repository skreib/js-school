function calculate() {
	...
	const departmentIndex = csvData.headers.indexOf('department');
	const monthIndex = csvData.headers.indexOf('month');

	let totalAmount = 0;
	let departmentBuffer = {};
	let monthBuffer = {};

	csvData.lines.forEach((el) => {
		let currentAmount = parseFloat(el[amountIndex]) || 0;
		let currentDepartment = el[departmentIndex] || '';
		let currentMonth = el[monthIndex] || '';

		if (currentDepartment) {
			if (departmentBuffer[currentDepartment]) {
				departmentBuffer[currentDepartment].push(currentAmount);
			} else {
				departmentBuffer[currentDepartment] = [currentAmount];
			}
		}

		if (currentMonth) {
			if (monthBuffer[currentMonth]) {
				monthBuffer[currentMonth].push(currentAmount);
			} else {
				monthBuffer[currentMonth] = [currentAmount];
			}
		}
	...
	let avgByDepartment = {}; 
	for (let prop in departmentBuffer) {
		let avg = departmentBuffer[prop].reduce((a, b) => a + b, 0) / departmentBuffer[prop].length;
		avgByDepartment[prop] = Math.round(parseFloat(avg) * 100) / 100;
	}

	let avgByMonth = {};
	for (let prop in monthBuffer) {
		let avg = monthBuffer[prop].reduce((a, b) => a + b, 0) / monthBuffer[prop].length;
		avgByMonth[prop] = Math.round(parseFloat(avg) * 100) / 100;
	}
...
function CSVtoArray(delimiter = ',', separator = '\n') {
	const csv = readFile();
    let allTextLines = csv.split(separator);
    let headers = allTextLines[0].split(delimiter).map(el => el.toLowerCase());
    let lines = [];

    for (let i = 1; i < allTextLines.length; i++) {
        let data = allTextLines[i].split(delimiter).map(el => el.trim());
